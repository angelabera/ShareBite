...
class ShareBiteFoodListing {
    constructor() {
        ...
        if (shared.format) {
            ...
            this.getUrgencyStatus = shared.format.getUrgencyStatus.bind(shared.format);
            this.getExpiryStatus = shared.format.getExpiryStatus.bind(shared.format);
        } else {
            ...
            this.getUrgencyStatus = function (freshUntil) { var now = new Date(); var expiry = new Date(freshUntil); var hoursLeft = (expiry - now) / (1000 * 60 * 60); if (hoursLeft > 6) return { label: '🟢 Fresh', className: 'fresh' }; if (hoursLeft > 2) return { label: '🟡 Expiring Soon', className: 'expiring' }; return { label: '🔴 Urgent', className: 'urgent' }; };
            this.getExpiryStatus = function (expiryDate) { var now = new Date(); var expiry = new Date(expiryDate); var hoursRemaining = (expiry - now) / (1000 * 60 * 60); if (hoursRemaining < 0) return 'expired'; if (hoursRemaining < 2) return 'expiring-soon'; return 'active'; };
        }
        if (shared.helpers) {
        ...
        if (this.validateFormData(formData)) {
            try {
                const backendData = {
                    ...
                    latitude: parseFloat(formData.latitude) || 0, // Ensure numeric
                    longitude: parseFloat(formData.longitude) || 0, // Ensure numeric
                };
    ...
    showToast(message, type) {
        ...
        toast.style.cssText = 'position:fixed;top:100px;right:20px;max-width:400px;min-height:auto;background:var(--primary-color);color:white;padding:1rem 1.5rem;border-radius:var(--border-radius);display:flex;align-items:center;gap:0.5rem;z-index:3000;box-shadow:var(--shadow-heavy);';
...
`       ;
        ...
        <div class="food-card ${isClaimed ? 'claimed' : ''} ${statusClass}"
            ...
            <div class="food-details" style="display: flex; flex-direction: column; flex-grow: 1; padding: 1.25rem;">
                ...
                <div class="card-actions" style="margin-top: 1rem;">
                    ${this.getClaimButtonHTML(listing, expiryStatus)}
                </div>
            </div>
        </div>
        `;
    ...
    loadClaimedItems() {
        return (window.ShareBiteShared && window.ShareBiteShared.storage) ? window.ShareBiteShared.storage.loadClaimedItems() : (localStorage.getItem('sharebite-claimed-items') ? JSON.parse(localStorage.getItem('sharebite-claimed-items')) : []);
    }

    saveClaimedItems() {
        if (window.ShareBiteShared && window.ShareBiteShared.storage) window.ShareBiteShared.storage.saveClaimedItems(this.claimedItems); else localStorage.setItem('sharebite-claimed-items', JSON.stringify(this.claimedItems));
    }

    loadNotifications() {
        return (window.ShareBiteShared && window.ShareBiteShared.storage) ? window.ShareBiteShared.storage.loadNotifications() : (localStorage.getItem('sharebite-notifications') ? JSON.parse(localStorage.getItem('sharebite-notifications')) : []);
    }

    saveNotifications() {
        if (window.ShareBiteShared && window.ShareBiteShared.storage) window.ShareBiteShared.storage.saveNotifications(this.notifications); else localStorage.setItem('sharebite-notifications', JSON.stringify(this.notifications));
    }
